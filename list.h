...
struct Node_s {
    ...
    void *node_item;
    int node_index; //use to determine index in array : use in deleting it and finding it
    Node *node_next;
    Node *node_previous;
};

//out of bound = beyond and after list
enum ListOutOfBounds {
    LIST_OOB_START,
    LIST_OOB_END,
    LIST_OOB_FALSE //list curr is in bound
};
...
struct List_s{
    ...
    Node *list_head;
    Node *list_tail;
    int list_index; //use to determine index in array : use in deleting it
    //bool list_out_of_bound;
    enum ListOutOfBounds list_bound;
    List *list_next;
};
